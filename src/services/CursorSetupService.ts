...
export class CursorSetupService {
  ...
  async run(): Promise<void> {
    ...
    if (action.label === 'Copy Claude Code Wrapper') {
      await vscode.env.clipboard.writeText('bellsy-run claude');
    ...
    if (action.label === 'Copy Generic Wrapper') {
      await vscode.env.clipboard.writeText('bellsy-run your-command-here');
    ...
    if (action.label === 'Copy Setup Checklist') {
      const checklist = [
        ...
        '2. Start your local tool through a wrapper like: bellsy-run claude',
